...
from pydantic import AnyHttpUrl, IPvAnyAddress, FilePath, errors as pydantic_errors
try:
    from pydantic import BaseSettings
    from pydantic.env_settings import env_file_sentinel as ENV_FILE_SENTINEL
except (pydantic_errors.PydanticImportError, ImportError):  # pip install pydantic-settings  when pydantic version >= 2.0 
...
class Settings(BaseSettings):
    ...
    # sql db
    SQL_USERNAME: Optional[str] = None   # 关系型数据库用户名
    SQL_PASSWORD: Optional[str] = None  # 关系型数据库用户密码
    SQL_HOST: Optional[str] = "127.0.0.1"    # 关系型数据库HOST地址, sqlite 可以SQL_HOST设为"/"，设置SQL_DATABASE为db文件的路径
    SQL_PORT: Optional[int] = 5432    # 关系型数据库端口
    SQL_DATABASE: Optional[str] = "dev"       # 关系型数据库数据库名
    SQL_TABLE_PREFIX: Optional[str] = 't_'  # 数据库表前缀， 不需要前缀可以置空
    SQLALCHEMY_ENGINE: str = 'mysql+aiomysql'   # SQL引擎需要加上异步引擎，修改此处可以快速改变SQL数据库(mysql+aiomysql postgres+asyncpg sqlite+aiosqlite...)

    def getSqlalchemyURL(self):
        """
        获取sqlachemy的连接语句, 有些数据库url可能不一样的时候请重写。
        """
        user = ""
        if self.SQL_USERNAME:
            user += self.SQL_USERNAME
        if self.SQL_PASSWORD:
            user += f":{self.SQL_PASSWORD}"
        if user:
            user += "@"
        url = f"{self.SQLALCHEMY_ENGINE}://{user}{self.SQL_HOST}"
        if self.SQL_PORT:
            url += f":{self.SQL_PORT}"
        if self.SQL_DATABASE:
            url += f"/{self.SQL_DATABASE}"
        return url
